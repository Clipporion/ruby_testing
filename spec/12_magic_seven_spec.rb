...
describe MagicSeven do
  ...
  describe '#subtract_four' do
    context 'when previous step is 16' do
      it 'returns 12' do
        previous_step = 16
  ...
  describe '#divide_by_two' do
    context 'if previous step is 12' do
      it 'returns 6' do
        previous_step = 12
  ...
  class_number = 3
  subject(:new_game) { described_class.new(class_number) }
  describe '#subtract_random_number' do
    context 'if previous step is 6' do
      it 'returns 3' do
        previous_step = 6
  ...
  describe '#play' do
    context 'when the random number is 22' do
      ...
      subject(:game) { described_class.new(22) }
      it 'will return 7' do
        result = game.play
    ...
    context 'when the random number is 156' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(156) }
      it 'will return 7' do
        result = game.play
    ...
    context 'when the random number is 2903' do
      # remove the 'x' before running this test
      subject(:game) { described_class.new(2903) }
      it 'will return 7' do
        result = game.play
        expect(result).to eq(7)
      end
